const char *pageLogin = R"raw(<!DOCTYPE html>
<html>
  <head>
  ...
  <body>
    ...
    <div class="login" style="display: block" id="myDiv">
      <form id="login">
        ...
        <select name="Ssid" id="selectSSIDS">
          <option id="def">Selecione sua Rede (ssid)</option>
        </select>
        <!-- <input type="text" name="Ssid" id="Ssid" placeholder="Informe o SSID" /> -->
        <br/><br/>
    ...
    <script>
      ...
      async function enviandoDados() {
        ...
        const response = fetch("http://192.168.4.1/senddata", {
          ...
          .then(dados => {
            if (dados != null) {
              window.location = "http://" + dados + "/autenticado";
            }
          })
          .catch((error) => {
            ShowPage();
